...
def help_menu():
    print('Usage:')
    # print(f'[{AUTH}]: Login using your Warframe.Market account credentials.')
...
def search_price():
    ...
    item = market_retrieve(sanitized_name)
    print_listing_info(item)
...
def market_retrieve(item_name: str) -> Item:
    ...
    if not orders or not items:
        return 'Error retrieving {item_name}. Did you spell it correctly?'
...
def _get_item_info(item_name: str):
    ...
    if res.status_code != requests.codes.ok:
        return None
    items = res.json()['payload']['item']['items_in_set']
    for item in items:
